  ...
  return (
    <div className="flex justify-center px-4 md:px-12 lg:px-24 xl:px-32 ">
        ...
        {value.length > 0 ? (
          ...
          >
          {value.map((item) => ( 
              <SwiperSlide key={`${item.image}`} className="relative flex items-center w-32rem h-15rem bg-black justify-center rounded-lg ">
                <img src={item.image} alt={`Slide ${item.publishedAt}`} className="w-full max-w-[1200px] h-[600px] flex flex-col justify-center items-center object-cover opacity-60 rounded-lg " />
